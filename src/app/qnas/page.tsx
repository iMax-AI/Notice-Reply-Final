...
export default function QuestionsPage() {
  ...
  const handleNumOfPagesChange = (value: string) => {
    ...
    if (!isNaN(numericValue)) {
      setNumOfPages(Math.max(2, numericValue));
    } else {
      setNumOfPages(2); // Reset to minimum of 2 if invalid or empty
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const filteredQuestions = questionsArr.filter((_, index) => selectedQuestions[index]).map(q => q.trim() + "?");
      const filteredAnswers = answers.filter((_, index) => selectedQuestions[index]).map(a => a.trim() + ".");
      ...
      // const saveQnAs = await fetch("/api/save-selected-qnas", {
      //   method: "POST",
      //   headers: {
      //     "Content-Type": "application/json",
      //   },
      //   body: JSON.stringify({ userID: userID, questions: String(filteredQuestions), answers: String(filteredAnswers) }),
      // });
      
      // console.log("saveQnAs", saveQnAs);
      ...
      if (response.status === 200) {
        const noticeResponse = await axios.get(
          "http://sastelaptop.com:3010/api/getNoticeReply",
          {
            params: { userID },
          }
        );
        ...
        if (noticeResponse.status === 200) {
          ...
          //     "Content-Type": "application/json",
          //   },
          //   body: JSON.stringify({ userID, notice: String(noticeData) }),
          // });
